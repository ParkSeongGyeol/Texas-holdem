...
class TightStrategy(Strategy):
    def decide(self, ai, community_cards, pot, current_bet, opponents):
        ...
        if stage == "pre":
            ...
            if pre >= 0.60: return Action.CALL, to_call
            if pre >= 0.55 and po <= 0.40: return Action.CALL, to_call
...
class LooseStrategy(Strategy):
    def decide(self, ai, community_cards, pot, current_bet, opponents):
        ...
        if stage == "pre":
            ...
            po = pot_odds(pot, to_call)
            if pre >= 0.50: return Action.CALL, to_call
            if pre >= 0.40 and po < 0.50: return Action.CALL, to_call
            return Action.FOLD, 0
        ...
        if strength >= 0.50:
            if to_call == 0: return Action.CHECK, 0
            if po <= 0.60: return Action.CALL, to_call
            return Action.FOLD, 0

        if strength >= 0.30:
            if to_call == 0: return Action.CHECK, 0
            if po <= 0.40: return Action.CALL, to_call
            return Action.FOLD, 0
